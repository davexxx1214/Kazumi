...
class ApiRuleStrategy {
  ...
  PreparedRuleRequest prepareRequest(
    ...
    if (uri.scheme != 'http' && uri.scheme != 'https') {
      throw ApiRuleFormatException('API 请求仅支持 HTTP(S): $url');
    }
    ...
    String? unsafeHeader;
    for (final header in config.headers.keys) {
      final normalized = header.toLowerCase();
      if (blockedHeaders.contains(normalized)) {
        unsafeHeader = normalized;
        break;
      }
    }
    if (unsafeHeader != null) {
      throw ApiRuleFormatException('API 请求不允许设置敏感请求头: $unsafeHeader');
    }
