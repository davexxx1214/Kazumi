...
class _SourceSheetViewState extends State<_SourceSheetView> {
  ...
  void _autoExpandFirstResult() {
    if (_autoExpansionHandled) return;
    if (isTV) {
      for (final group in widget.groups) {
        if (group.name.toUpperCase() == 'AGE') {
          _autoExpansionHandled = true;
          _display[group.name] = _SourceDisplay.preview;
          return;
        }
      }
    }
  ...
  Widget _buildResult(_SourceSearchGroup group, int index) {
    ...
    return SplitListRow(
      autofocus: isTV && index == 0 &&
          (group.name.toUpperCase() == 'AGE' ||
              (!widget.groups.any((source) => source.name.toUpperCase() == 'AGE') &&
                  group.name == widget.firstResultSource)),
