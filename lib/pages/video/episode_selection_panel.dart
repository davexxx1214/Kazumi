  ...
  void _selectRoad(int road) {
    ...
    if (isTV) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && widget.roads[road].data.isNotEmpty) {
          _focusNodeFor(road, 1).requestFocus();
        }
      });
    }
  }

  FocusNode _focusNodeFor(int road, int episode) => _episodeFocusNodes.putIfAbsent(
        '$road:$episode',
        () => FocusNode(debugLabel: 'Episode $road:$episode menu card'),
      );

  Future<void> focusCurrentEpisode() async {
    if (!isTV || !_canLocate) return;
    await revealCurrentEpisode();
    if (!mounted) return;
    await WidgetsBinding.instance.endOfFrame;
    if (mounted) {
      _focusNodeFor(widget.selectedRoad, widget.selectedEpisode).requestFocus();
    }
  }
                  ...
                  sliver: SliverList.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _EpisodeRow(
                        ...
                        name: name,
                        focusNode: isTV ? _focusNodeFor(_visibleRoad, episode) : null,
                        first: index == 0,
                        last: index == count - 1,
