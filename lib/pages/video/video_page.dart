...
class _VideoPageState extends State<VideoPage>
  ...
  void _focusCurrentEpisode() {
    if (!isTV) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        unawaited(_episodePanelKey.currentState?.focusCurrentEpisode());
      }
    });
  }
  ...
  KeyEventResult _handleSidePanelKeyEvent(FocusNode node, KeyEvent event) {
    if (!isTV || event is! KeyDownEvent) return KeyEventResult.ignored;
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: !_isSidePanelOpen,
      onPopInvokedWithResult: (bool didPop, Object? result) {
        if (!didPop && _isSidePanelOpen) {
          _closeSidePanel();
          return;
        }
        if (didPop) {
            ...
            body: VideoPageLayout(
              ...
              sidePanel: VideoSidePanel(
                ...
                child: Focus(
                  ...
                  onKeyEvent: _handleSidePanelKeyEvent,
                  child: tabBody,
                ),
              ),
            ),
